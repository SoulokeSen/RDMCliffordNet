...
#        self.select_subspace = select_subspace
 #this list index has to be the same order as that of input_grades in algebra
# add option for making extracting a subsubspace dimension from the algebra.n_subspaces and self.algebra.subspaces
        if subspaces:
...
#        torch.nn.init.normal_(self.weight, std=math.sqrt(2 / self.in_features))
    ...
    def _forward_subspaces(self, input):
        ...
        weight = self.weight.repeat_interleave(self.algebra.subspaces
                                               , dim=-1)
    ...
    def forward(self, input):
        ...
        if self.bias is not None:  
                bias = self.algebra.embed_sub(self.bias, self.b_dims)   #in case you need bias for subsubpace, just add it without embedding
