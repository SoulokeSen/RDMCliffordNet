...
class Logger:
    ...
    def log_matplotlib(self, *args, **kwargs):
        return

    def log_point_cloud(self, *args, **kwargs):
        return
...
class WANDBLogger:
    ...
    def log_point_cloud(self, object_dict, step):

        for k, v in object_dict.items():
            if isinstance(v, torch.Tensor):
                v = v.detach().cpu().numpy()
            object_dict[k] = wandb.Object3D(v)

        return self._log(object_dict, step)

    def log_matplotlib(self, image_dict, step):
